...
- **Backend** — Python 3.13 + FastAPI, SQLite for persistence, **argon2id** (`argon2-cffi`) for passwords, JWT (HS256) carried in a HttpOnly cookie.
- **Frontend** — React 18 + TypeScript SPA (Vite), `react-hook-form` + `zod` for validation, Context API for auth state, native `fetch` (no axios).
...
│   │   ├── services/       # password (argon2id), token (JWT)
│   │   └── routers/        # auth, me
...
| `ARGON2_TIME_COST`   | `3`                      | argon2id iterations. Tests override to `1`.                      |
| `ARGON2_MEMORY_COST` | `65536`                  | argon2id memory in KiB (64 MiB). Tests override to `8`.          |
| `ARGON2_PARALLELISM` | `4`                      | argon2id parallelism. Tests override to `1`.                     |
...
- argon2id round-trip, salt uniqueness, malformed-hash safety (incl. rejection of bcrypt-format hashes).
...
- **Modern password hashing.** argon2id (PHC 2015 winner, OWASP first recommendation 2024) — memory-hard, GPU-resistant, no 72-byte truncation.
...
- **No silent email enumeration.** Sign-in returns the same generic error for "no such user" and "wrong password", and runs argon2id unconditionally against a pre-baked dummy hash when the user does not exist — so the response time can't be used as a side channel either.
- **Strict TypeScript.** `strict`, `noUnusedLocals`, `noUncheckedIndexedAccess`, `verbatimModuleSyntax`.
- **Native `fetch` over axios.** A 30-line typed wrapper handles JSON / credentials / error envelope; ~14 KB smaller bundle.
